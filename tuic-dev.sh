...
install_pkg() {
    ...
    cert_update() {
        ...
        if [ -f "/root/.acme.sh/acme.sh" ]; then
            ...
            msg err "acme.sh is not installed. Please run the 'install' command first."
            exit 1
        fi
    }
    ...
    apply_cert() {
        ...
        if [ -f "${workspace}/fullchain.pem" ] && [ -f "${workspace}/private_key.pem" ]; then
            ...
            msg err "Certificate or private key file not found. Please make sure they exist in the specified path."
            exit 1
        fi
    }
